...
#include <APL/types/BracketTok.h>
...
const std::vector<wchar_t> Tokenizer::validNames = std::vector<wchar_t>((std::wstring(L"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_∆")).begin(), (std::wstring(L"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_∆")).end());
const std::wstring Tokenizer::ops = L"⍺⍳⍴⍵!%*+,-./<=>?@\\^|~⍬⊢⊣⌷¨⍨⌿⍀≤≥≠∨∧÷×∊↑↓○⌈⌊∇∘⊂⊃∩∪⊥⊤⍱⍲⍒⍋⍉⌽⊖⍟⌹⍕⍎⍫⍪≡≢⍷→⎕⍞⍣⍶⍸⍹⌸⌺⍇⍢⍤⍁⍂⊆⊇⊙⌾⌻⌼⍃⍄⍅⍆⍈⍊⍌⍍⍏⍐⍑⍓⍔⍖⍗⍘⍚⍛⍜⍠⍡⍥⍦⍧⍩⍭⍮⍯⍰√‽⊗ϼ∍⋾…ᑈᐵ";
	...
	Tokenizer::Line::Line(const std::wstring &line, int pos, std::vector<std::shared_ptr<Token>> &ts) : ts(ts), line(line), pos(pos)
	{
	}

	Tokenizer::Line::Line(const std::wstring &line, int pos) : Line(line, pos, std::vector<std::shared_ptr<Token>>())
	{
	}
	...
	std::shared_ptr<LineTok> Tokenizer::Line::tok()
	{
	  if (annoyingBacktickPos)
	  {
	  ...
	  return std::make_shared<LineTok>(line, spos, epos, ts);
	}
	...
	std::wstring Tokenizer::Block::toString()
	{
	  return L"<" + a + L"," + StringHelper::toString(b) + L">";
	}

	std::shared_ptr<BasicLines> Tokenizer::tokenize(const std::wstring &raw)
	{
	  return tokenize(raw, false);
	}

	std::shared_ptr<BasicLines> Tokenizer::tokenize(const std::wstring &raw, bool pointless)
	{ // pointless means unevaled things get tokens; mainly for syntax highlighting
	  ...
	  for (int i = 0; i < len; li = i)
	  {
		...
		try
		{
		  ...
		  std::wstring cS = StringHelper::toString(c);
		  if (c == L'(' || c == L'{' || c == L'[')
		  {
			wchar_t match;
			switch (c)
		  ...
		  else if (c == L')' || c == L'}' || c == L']')
		  {
			...
			auto lineTokens = std::vector<std::shared_ptr<LineTok>>();
			...
			{
			  case L')':
				r = std::make_shared<ParenTok>(raw, closed->pos, i + 1, lineTokens, closed->hasDmd);
				break;
			  case L'}':
				r = std::make_shared<DfnTok>(raw, closed->pos, i + 1, lineTokens);
				break;
			  case L']':
				r = std::make_shared<BracketTok>(raw, closed->pos, i + 1, lineTokens, closed->hasDmd);
				break;
			  default:
		  ...
		  else if (c >= L'0' && c <= L'9' || c == L'¯' || c == L'.' && next >= L'0' && next <= L'9')
		  {
			...
			if (i < len)
			{
			  ...
			  if (i < len && raw[i] == L'L' && (i + 1 == len || !validNameMid(raw[i + 1])))
			  {
				...
				tokens->add(std::make_shared<BigTok>(raw, li, i, std::make_shared<BigValue>(big)));
			  }
			  else
			  {
		  ...
		  else if (ops.find(cS) != std::wstring::npos)
		  {
		  ...
		  else
		  {
			...
			{
			  std::wstring hex = Integer::toHexString(c);
	  ...
	  if (levels.size() != 1)
	  {
		...
		while (levels.size() > 1)
		{
		  ...
		  {
			case L')':
			  r = std::make_shared<ParenTok>(raw, closed->pos, len, lineTokens, closed->hasDmd);
			  break;
			case L'}':
			  r = std::make_shared<DfnTok>(raw, closed->pos, len, lineTokens);
			  break;
			case L']':
			  r = std::make_shared<BracketTok>(raw, closed->pos, len, lineTokens, closed->hasDmd);
			  break;
			default:
	  ...
	  auto expressions = std::vector<std::shared_ptr<LineTok>>();
	  for (auto line : lines)
	  {
		expressions.push_back(line->tok());
	  }
	  return std::make_shared<BasicLines>(raw, 0, len, expressions);
	}
}
