...
namespace APL::types::dimensions
{
    ...
    Brackets::Brackets(std::shared_ptr<APL::types::Value> val) : Callable(nullptr), val(val)
    {
    }
    ...
    std::string Brackets::toString()
    {
      return L"[" + val + L"]";
    }

    std::shared_ptr<APL::types::Obj> Brackets::of(std::shared_ptr<APL::tokenizer::types::BracketTok> t, std::shared_ptr<Scope> sc)
    {
      if (t->array)
      {
        std::vector<std::shared_ptr<APL::types::Value>> lns(t->tokens.size());
        for (int i = 0; i < t->tokens.size(); i++)
        {
          std::shared_ptr<APL::tokenizer::types::LineTok> tk = t->tokens[i];
      ...
      else
      {
        ...
        std::shared_ptr<APL::types::Value> res = Main::vexec(t->tokens[0], sc);
