...
namespace APL::types::arrs
{
	...
	BitArr::BitArr(std::vector<long long> &arr, std::vector<int> &shape) : Arr(shape), arr(arr)
	{
	  assert((Main::enclosePrimitives || shape.size() != 0, L"Internal: attempting to create a BitArr of shape ⍬"));
	  assert((sizeof_Keyword(shape) == arr.size(), std::to_wstring(arr.size()) + L" not expected for shape " + Main::formatAPL(shape)));
	}

	BitArr::BitArr(std::vector<long long> &arr, std::vector<int> &shape, int ia) : Arr(shape, ia), arr(arr)
	{
	}

	std::shared_ptr<BitArr> BitArr::of(std::shared_ptr<Arr> a)
	{
	  if (std::dynamic_pointer_cast<BitArr>(a) != nullptr)
	  {
		  return std::static_pointer_cast<BitArr>(a);
	  }
	  if (a->quickDoubleArr())
	  {
		std::vector<double> ds = a->asDoubleArr();
		std::vector<long long> arr = convert(ds);
		return std::make_shared<BitArr>(arr, a->shape);
	  }
	...
	std::vector<long long> BitArr::convert(std::vector<double> &arr)
	{
	...
	int BitArr::sizeof_Keyword(std::shared_ptr<Value> w)
	{
	  return w->ia + 63 >> 6;
	}

	int BitArr::sizeof_Keyword(std::vector<int> &sh)
	{
	  int m = 1;
	  for (auto i : sh)
	  {
	...
	int BitArr::sizeof_Keyword(int am)
	{
	  return am + 63 >> 6;
	}

	std::shared_ptr<Value> BitArr::fill(std::shared_ptr<Value> v, bool b)
	{
	  std::vector<long long> arr(sizeof_Keyword(v));
	  if (!b)
	  {
		  return std::make_shared<BitArr>(arr, v->shape, v->ia);
	  }
	  Arrays::fill(arr, -1LL);
	  return std::make_shared<BitArr>(arr, v->shape, v->ia);
	}

	std::vector<int> BitArr::asIntArrClone()
	{
	...
	std::vector<double> BitArr::asDoubleArr()
	{
	...
	std::shared_ptr<Value> BitArr::get(int i)
	{
	  return Num::NUMS[static_cast<int>((arr[i >> 6] >> (i & 63)) & 1)]; // no branching!
	}

	std::wstring BitArr::asString()
	{
	  throw DomainError(L"Using bit array as string", shared_from_this());
	}

	std::shared_ptr<Value> BitArr::prototype()
	{
	  return Num::ZERO;
	}

	std::shared_ptr<Value> BitArr::safePrototype()
	{
	  return Num::ZERO;
	}

	std::shared_ptr<Value> BitArr::ofShape(std::vector<int> &sh)
	{
	  if (sh.empty() && !Main::enclosePrimitives)
	  {
		  return this->operator[](0);
	  }
	  return std::make_shared<BitArr>(arr, sh);
	}

	bool BitArr::quickDoubleArr()
	{
	  return true;
	}
	...
	BitArr::BA::BA(int am) : a(std::vector<long long>(sizeof_Keyword(am))), sh(std::vector<int>{am})
	{
	}

	BitArr::BA::BA(std::vector<int> &sh) : a(std::vector<long long>(sizeof_Keyword(sh))), sh(sh)
	{
	}

	BitArr::BA::BA(std::vector<long long> &a, int start, std::vector<int> &sh) : a(a), sh(sh)
	{
	  i = start >> 6;
	  o = start & 63;
	}

	void BitArr::BA::add(bool b)
	{
	  a[i] |= (b? 1LL : 0LL) << o;
	  o++;
	...
	void BitArr::BA::add(long long l)
	{ // expects a 0 or 1
	  a[i] |= l << o;
	  o++;
	  if (o == 64)
	...
	void BitArr::BA::add(std::shared_ptr<BitArr> a)
	{
	  add(a, 0, a->ia);
	}

	void BitArr::BA::add(std::shared_ptr<BitArr> g, int s, int e)
	{
	  if (s == e)
	  {
		  return;
...
//ORIGINAL LINE: @SuppressWarnings("ConstantExpression") public void fill(int n)
	...
	std::shared_ptr<BitArr> BitArr::BA::finish()
	{
	  return std::make_shared<BitArr>(a, sh);
	}

	std::wstring BitArr::str64(long long l)
	{
	...
	long long BitArr::longFrom(int s)
	{
	  int i1 = s >> 6;
	  int i2 = (s + 63) >> 6;
	  int o1 = s & 63;
	...
	BitArr::BC::BC(std::vector<int> &sz) : arr(std::vector<long long>(sizeof_Keyword(sz))), sz(sz)
	{
	}

	std::shared_ptr<BitArr> BitArr::BC::finish()
	{
	  // assert (i<<6) + o == Arr.prod(sz); \\ idk man
	  return std::make_shared<BitArr>(arr, sz);
	}

	void BitArr::BC::set(int pos)
	{
	  arr[pos >> 6] |= 1LL << (pos & 63);
	}

	void BitArr::BC::clear(int pos)
	{
	  arr[pos >> 6] &= ~(1LL << (pos & 63));
	}

	std::shared_ptr<BC> BitArr::create(std::vector<int> &sh)
	{
	...
	std::shared_ptr<Value> BitArr::squeeze()
	{
	  return shared_from_this(); // we don't need no squeezing!
	}

	std::shared_ptr<BR> BitArr::read()
	{
	  return std::make_shared<BR>(shared_from_this(), shared_from_this());
	}

	std::vector<std::shared_ptr<Value>> BitArr::valuesCopy()
	{
