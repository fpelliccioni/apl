...
const std::vector<double> DoubleArr::EMPTY = std::vector<double>(0);
	...
	std::shared_ptr<Value> DoubleArr::safe(std::vector<double> &vs, std::vector<int> &sh)
	{
	...
	std::shared_ptr<Value> DoubleArr::get(int i)
	{
	  return Num::of(arr[i]);
	}

	std::shared_ptr<Value> DoubleArr::first()
	{
	  if (ia > 0)
	  {
	...
	std::wstring DoubleArr::asString()
	{
	  throw DomainError(L"Using number array as string", shared_from_this());
	}

	std::shared_ptr<Value> DoubleArr::prototype()
	{
	  return Num::ZERO;
	}

	std::shared_ptr<Value> DoubleArr::safePrototype()
	{
	  return Num::ZERO;
	}

	std::shared_ptr<Value> DoubleArr::ofShape(std::vector<int> &sh)
	{
	  if (sh.empty() && !Main::enclosePrimitives)
	  {
	...
	std::shared_ptr<Value> DoubleArr::squeeze()
	{
	  return shared_from_this();
	}

	std::vector<std::shared_ptr<Value>> DoubleArr::valuesCopy()
	{
	...
	bool DoubleArr::equals(std::shared_ptr<Obj> o)
	{
