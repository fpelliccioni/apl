...
const std::vector<int> EmptyArr::NOINTS = std::vector<int>(0);
    ...
    EmptyArr::EmptyArr(std::vector<int> &sh, std::shared_ptr<APL::types::Value> proto) : Arr(sh, 0, sh.size()), proto(proto)
    {
    }
    ...
    std::shared_ptr<APL::types::Value> EmptyArr::get(int i)
    {
      throw ImplementationError(L"internal: using get() on empty array; view )stack");
    }

    std::string EmptyArr::asString()
    {
      if (rank >= 2)
      {
    ...
    std::shared_ptr<APL::types::Value> EmptyArr::prototype()
    {
      if (proto->empty())
      {
    ...
    std::shared_ptr<APL::types::Value> EmptyArr::safePrototype()
    {
      return proto;
    }

    std::shared_ptr<APL::types::Value> EmptyArr::ofShape(std::vector<int> &sh)
    {
      assert(ia == Arr::prod(sh));
      return std::make_shared<EmptyArr>(sh, proto);
    }

const std::vector<std::shared_ptr<APL::types::Value>> EmptyArr::NO_VALUES = std::vector<std::shared_ptr<APL::types::Value>>(0);

    std::vector<std::shared_ptr<APL::types::Value>> EmptyArr::valuesCopy()
    {
      return NO_VALUES; // safe, copy or not - doesn't matter
    }

    std::shared_ptr<APL::types::Value> EmptyArr::squeeze()
    {
      return shared_from_this();
    }

//NOTE(fernando)
// const std::shared_ptr<java::util::Iterator<std::shared_ptr<APL::types::Value>>> EmptyArr::EIT = std::make_shared<IteratorAnonymousInnerClass>();
    ...
    std::shared_ptr<APL::types::Value> EmptyArr::IteratorAnonymousInnerClass::next()
    {
        throw IllegalStateException("iterating empty array");
    }

    //NOTE(fernando)
    // std::shared_ptr<Iterator<std::shared_ptr<APL::types::Value>>> EmptyArr::iterator()
    // {
    //   return EIT;
    // }
}
