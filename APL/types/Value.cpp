...
namespace APL::types
{
    ...
    APL::types::Value::APL::types::Value(std::vector<int> &shape) : shape(shape), rank(shape.size())
    {
      int tia = 1;
    ...
    APL::types::Value::APL::types::Value(std::vector<int> &shape, int ia, int rank) : shape(shape), rank(rank), ia(ia)
    {
    }

    std::vector<int> APL::types::Value::asIntVec()
    { // succeeds on rank ≤ 1
      if (rank > 1)
      {
    ...
    std::vector<int> APL::types::Value::asIntArr()
    {
      return asIntArrClone();
    }

    bool APL::types::Value::scalar()
    {
      return rank == 0;
    }

    std::shared_ptr<APL::types::Value> APL::types::Value::first()
    {
      return this->operator[](0);
    }

    int APL::types::Value::compareTo(std::shared_ptr<APL::types::Value> r)
    {
      std::shared_ptr<APL::types::Value> l = shared_from_this();
      ...
      if (std::dynamic_pointer_cast<APL::types::BigValue>(l) != nullptr && std::dynamic_pointer_cast<APL::types::BigValue>(r) != nullptr)
      {
          return (std::static_pointer_cast<APL::types::BigValue>(l))->i->compareTo((std::static_pointer_cast<APL::types::BigValue>(r))->i);
      }
      if (!lA && !rA)
      {
    ...
    std::vector<std::optional<int>> APL::types::Value::gradeUp()
    {
      if (rank != 1)
      {
    ...
    std::vector<std::optional<int>> APL::types::Value::gradeDown()
    {
      if (rank != 1)
      {
          throw DomainError(L"grading rank ≠ 1", shared_from_this());
    ...
    std::vector<int> APL::types::Value::eraseDim(int place)
    {
    ...
    APL::Type APL::types::Value::type()
    {
      return Type::array;
    }

    std::string APL::types::Value::oneliner(std::vector<int> &where)
    {
      return toString();
    }

    std::string APL::types::Value::oneliner()
    {
      return oneliner(EmptyArr::NOINTS);
    }

    std::vector<std::shared_ptr<APL::types::Value>> APL::types::Value::values()
    {
      return valuesCopy();
    }

    std::vector<std::shared_ptr<APL::types::Value>> APL::types::Value::valuesCopy()
    {
    ...
    std::shared_ptr<Iterator<std::shared_ptr<APL::types::Value>>> APL::types::Value::iterator()
    {
      return std::make_shared<ValueIterator>(shared_from_this());
    }

    std::shared_ptr<APL::types::Value> APL::types::Value::ind(std::vector<std::vector<double>> &ind, int id, int IO)
    {
      if (ind.size() != rank)
      {
    ...
    APL::types::Value::ValueIterator::ValueIterator(std::shared_ptr<APL::types::Value> outerInstance) : outerInstance(outerInstance)
    {
    }

    bool APL::types::Value::ValueIterator::hasNext()
    {
      return c < outerInstance->ia;
    }

    std::shared_ptr<APL::types::Value> APL::types::Value::ValueIterator::next()
    {
      return get(c++);
    }

    std::shared_ptr<APL::types::Value> APL::types::Value::at(std::vector<int> &pos, int IO)
    {
      if (pos.size() != rank)
      {
    ...
    std::shared_ptr<APL::types::Value> APL::types::Value::at(std::vector<int> &pos, std::shared_ptr<APL::types::Value> def)
    { // 0-indexed
      int x = 0;
      for (int i = 0; i < rank; i++)
      {
    ...
    std::shared_ptr<APL::types::Value> APL::types::Value::simpleAt(std::vector<int> &pos)
    {
      int x = 0;
      for (int i = 0; i < rank; i++)
      {
    ...
    std::vector<double> APL::types::Value::asDoubleArr()
    { // warning: also succeeds on a primitive number; don't modify
      std::vector<double> res(ia);
      int i = 0;
      for (std::shared_ptr<APL::types::Value> c : values())
      {
        res[i++] = c->asDouble();
      }
      return res;
    ...
    std::vector<double> APL::types::Value::asDoubleArrClone()
    {
      return asDoubleArr().clone();
    }

    double APL::types::Value::asDouble()
    {
      throw DomainError(StringHelper::wstring_to_string(L"Using " + this->humanType(true) + L" as a number", shared_from_this()));
    }

    bool APL::types::Value::quickDoubleArr()
    { // if true, asDoubleArr must succeed; warning: also succeeds on a primitive number
      return false;
    }

    std::shared_ptr<APL::types::Value> APL::types::Value::squeeze()
    {
      if (ia == 0)
      {
          return shared_from_this();
      }
      std::shared_ptr<APL::types::Value> f = this->operator[](0);
      ...
      if (std::dynamic_pointer_cast<Char>(f) != nullptr)
      {
        ...
        if (s != nullptr)
        {
            return std::make_shared<APL::types::arrs::ChrArr>(s->toString(), shape);
        }
      }
      bool anyBetter = false;
      std::vector<std::shared_ptr<APL::types::Value>> optimized(ia);
      std::vector<std::shared_ptr<APL::types::Value>> values = values();
      for (int i = 0, valuesLength = values.size(); i < valuesLength; i++)
      {
        std::shared_ptr<APL::types::Value> v = values[i];
        std::shared_ptr<APL::types::Value> vo = v->squeeze();
        if (vo != v)
        {
            anyBetter = true;
        }
    ...
    std::shared_ptr<APL::types::Value> APL::types::Value::cut(int s, int len, std::vector<int> &sh)
    {
      if (len == 1)
      {
        std::shared_ptr<APL::types::Value> v = this->operator[](s);
      ...
      std::vector<std::shared_ptr<APL::types::Value>> w = values();
      std::vector<std::shared_ptr<APL::types::Value>> r(len);
      std::copy_n(w.begin() + s, len, r.begin());
