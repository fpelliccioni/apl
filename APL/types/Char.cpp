...
const std::shared_ptr<Char> Char::SPACE = ASCII[L' '];
	...
	std::wstring Char::oneliner(std::vector<int> &ignored)
	{
	  return L"'" + StringHelper::toString(chr) + L"'";
	}

	std::shared_ptr<Value> Char::ofShape(std::vector<int> &sh)
	{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
	  return std::make_shared<ChrArr>(Character::toString(chr), sh); // special because there's no quickCharArr
	}
	...
	std::shared_ptr<Value> Char::safePrototype()
	{
	  return SPACE;
	}
}
