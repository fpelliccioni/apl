...
const std::vector<int> Primitive::SHAPE = std::vector<int>(0);
	...
	Primitive::Primitive() : Value(SHAPE, 1, 0)
	{
	}

	std::shared_ptr<Value> Primitive::get(int i)
	{
	  return shared_from_this();
	}

	std::shared_ptr<Value> Primitive::first()
	{
	  return shared_from_this();
	}
	...
	std::wstring Primitive::asString()
	{
	  throw DomainError(StringHelper::wstring_to_string(L"Using " + this->oneliner() + L" as string", shared_from_this()));
	}

	std::shared_ptr<Value> Primitive::prototype()
	{
	  std::shared_ptr<Value> v = safePrototype();
	  if (v->empty())
	  {
	...
	std::shared_ptr<Value> Primitive::safePrototype()
	{
	  return nullptr;
	}

	std::shared_ptr<Value> Primitive::squeeze()
