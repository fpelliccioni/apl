...
namespace APL::types::functions::trains
{
	...
	Atop::Atop(std::shared_ptr<Obj> g, std::shared_ptr<Fun> h) : g(g), h(h)
	{
	}

	std::shared_ptr<Value> Atop::call(std::shared_ptr<Value> w)
	{
	  if (std::dynamic_pointer_cast<Fun>(g) != nullptr)
	  {
		  return (std::static_pointer_cast<Fun>(g))->call(h->call(w));
	  }
	  return h->call(std::static_pointer_cast<Value>(g), w);
	}

	std::shared_ptr<Value> Atop::callInv(std::shared_ptr<Value> w)
	{
	  ...
	  return h->callInvW(std::static_pointer_cast<Value>(g), w);
	}

	std::shared_ptr<Value> Atop::call(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  if (!(std::dynamic_pointer_cast<Fun>(g) != nullptr))
	  {
	...
	std::shared_ptr<Value> Atop::callInvW(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  if (!(std::dynamic_pointer_cast<Fun>(g) != nullptr))
	  {
	...
	std::shared_ptr<Value> Atop::callInvA(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  if (!(std::dynamic_pointer_cast<Fun>(g) != nullptr))
	  {
		  throw SyntaxError(L"inverting a dyadically called A f train");
	...
	std::shared_ptr<Value> Atop::under(std::shared_ptr<Obj> o, std::shared_ptr<Value> w)
	{
	  if (std::dynamic_pointer_cast<Fun>(g) != nullptr)
	  {
	  ...
	  {
		return h->underW(o, std::static_pointer_cast<Value>(g), w);
	  }
	}
	...
	std::wstring Atop::FunAnonymousInnerClass::repr()
	{
		return gf->repr();
	}

	std::shared_ptr<Value> Atop::FunAnonymousInnerClass::call(std::shared_ptr<Value> w)
	{
		return gf->under(o, w);
	}

	std::wstring Atop::repr()
	{
	  return L"(" + g + L" " + h + L")";
	}
}
