...
std::string FindBuiltin::repr()
{
	return L"⍷";
}

std::shared_ptr<APL::types::Value> FindBuiltin::call(std::shared_ptr<APL::types::Value> a, std::shared_ptr<APL::types::Value> w)
{
	if (a->rank != w->rank)
	{
	...
	std::shared_ptr<APL::types::arrs::BitArr::BC> bc = std::make_shared<APL::types::arrs::BitArr::BC>(w->shape);
	if (a->rank == 1)
	{
	if (std::dynamic_pointer_cast<APL::types::arrs::BitArr>(a) != nullptr && std::dynamic_pointer_cast<APL::types::arrs::BitArr>(w) != nullptr)
	{
		std::vector<long long> ab = (std::static_pointer_cast<APL::types::arrs::BitArr>(a))->arr;
		std::vector<long long> wb = (std::static_pointer_cast<APL::types::arrs::BitArr>(w))->arr;
	...
	for (auto inW : ind)
	{
		...
		{
		std::shared_ptr<APL::types::Value> vA = a->simpleAt(inA);
		std::shared_ptr<APL::types::Value> vW = w->simpleAt(Indexer::add(inA, inW));
		if (!vA->equals(vW))
		{
			goto wContinue;
		}
