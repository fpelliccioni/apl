...
namespace APL::types::functions::builtins::mops
{
    ...
    std::string ObliqueBuiltin::repr()
    {
      return L"⍁";
    }

    std::shared_ptr<APL::types::Value> ObliqueBuiltin::call(std::shared_ptr<APL::types::Obj> f, std::shared_ptr<APL::types::Value> w, std::shared_ptr<DerivedMop> derv)
    {
      if (w->rank != 2)
      {
      ...
      if (w->quickDoubleArr())
      {
        ...
        for (int i = 0; i < ram; i++)
        {
          std::shared_ptr<APL::types::Value> v = ff->call(std::make_shared<DoubleArr>(rows[i]));
          if (v->rank != rrank)
          {
      ...
      else
      {
        ...
        std::vector<std::vector<std::shared_ptr<APL::types::Value>>> rows(ram);
        for (int i = 0; i < ram; i++)
        {
          rows[i] = std::vector<std::shared_ptr<APL::types::Value>>(i < szm? i + 1 : i >= szM? szm + szM - i - 1 : szm);
        }
        int p = 0;
        for (int y = 0; y < H; y++)
        {
          for (int x = 0; x < W; x++)
          {
            std::shared_ptr<APL::types::Value> v = vals[p++];
            int ri = x + y;
            int s = ri > W - 2? y + W - ri - 1 : y;
            rows[ri][s] = v;
          }
        ...
        for (int i = 0; i < ram; i++)
        {
          std::shared_ptr<APL::types::Value> v = ff->call(std::make_shared<HArr>(rows[i]));
          if (v->rank != rrank)
          {
