...
namespace APL::types::functions::builtins::mops
{
    ...
    std::string TableBuiltin::repr()
    {
      return L"∘.";
    }

    std::shared_ptr<APL::types::Value> TableBuiltin::call(std::shared_ptr<APL::types::Obj> f, std::shared_ptr<APL::types::Value> a, std::shared_ptr<APL::types::Value> w, std::shared_ptr<DerivedMop> derv)
    {
      ...
      int i = 0;
      std::shared_ptr<APL::types::Value> first = ff->call(a->first(), w->first());
      ...
      if (std::dynamic_pointer_cast<Num>(first) != nullptr)
      {
        ...
        std::shared_ptr<APL::types::Value> failure = nullptr;

        for (auto na : a)
        {
          for (auto nw : w)
          {
            std::shared_ptr<APL::types::Value> r;
            if (firstSkipped)
            {
                r = ff->call(na, nw);
            }
        ...
        { // i points to the place the failure should be
          std::vector<std::shared_ptr<APL::types::Value>> res(a->ia * w->ia);
          for (int n = 0; n < i; n++)
          ...
          if (i % w->ia != 0)
          { // finish the damn row..
            std::shared_ptr<APL::types::Value> va = a[i / w->ia];
          ...
          { // and do the rest, slowly and horribly
            std::shared_ptr<APL::types::Value> va = a[ai];
            for (auto vw : w)
            {
              res[i++] = ff->call(va, vw);
            }
      ...
      bool firstSkipped = false;
      std::vector<std::shared_ptr<APL::types::Value>> arr(a->ia * w->ia);
      for (auto na : a)
      {
        for (auto nw : w)
        {
