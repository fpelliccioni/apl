...
namespace APL::types::functions::builtins::mops
{
	...
	std::wstring ReduceBuiltin::repr()
	{
	  return L"/";
	}

	std::shared_ptr<Value> ReduceBuiltin::call(std::shared_ptr<Obj> f, std::shared_ptr<Value> w, int dim)
	{
	...
	std::shared_ptr<Value> ReduceBuiltin::call(std::shared_ptr<Obj> f, std::shared_ptr<Value> w, std::shared_ptr<DerivedMop> derv)
	{
	  if (!(std::dynamic_pointer_cast<Fun>(f) != nullptr))
	  {
		  throw SyntaxError(L"/ is only reduce. To use as replicate, use ⌿", f);
	  ...
	  if (std::dynamic_pointer_cast<CatBuiltin>(f) != nullptr)
	  {
		...
		{
		  {
			std::shared_ptr<Value> first = w->first();
			int am = 0;
			int chki = 0;
			{
			  if (std::dynamic_pointer_cast<ChrArr>(first) != nullptr || std::dynamic_pointer_cast<Char>(first) != nullptr)
			  {
				for (auto v : w)
				{
				  if (v->rank > 1)
				  ...
				  if (!(std::dynamic_pointer_cast<ChrArr>(v) != nullptr || std::dynamic_pointer_cast<Char>(v) != nullptr))
				  {
					  goto typedBreak;
				  }
				  am += v->ia;
				...
				return Main::toAPL(std::wstring(cs));
			...
			std::vector<std::shared_ptr<Value>> vs(am);
			int ri = 0;
			for (auto v : w)
			{
	  ...
	  std::vector<std::shared_ptr<Value>> a = w->values();
	  if (a.empty())
	  {
		std::shared_ptr<Value> id = ff->identity();
		if (id->empty())
		{
	  ...
	  std::shared_ptr<Value> last = a[a.size() - 1];
	...
	std::shared_ptr<Value> ReduceBuiltin::call(std::shared_ptr<Obj> f, std::shared_ptr<Value> a, std::shared_ptr<Value> w, std::shared_ptr<DerivedMop> derv)
	{
	  std::shared_ptr<Fun> ff = isFn(f);
	  if (w->rank != 1)
	  {
		  throw NYIError(L"A f/ B with 2≤⍴⍴B hasn't been implemented", shared_from_this(), w);
	  }
	  int n = a->asInt();
	  std::vector<std::shared_ptr<Value>> ra(w->ia - std::abs(n) + 1);
	  std::vector<std::shared_ptr<Value>> wa = w->values();
	  if (n > 0)
	  {
		for (int i = 0; i < ra.size(); i++)
		{
		  std::shared_ptr<Value> r = wa[i + n - 1];
		  for (int j = n - 2; j >= 0; j--)
		  {
			r = ff->call(wa[i + j], r);
		  }
	...
	std::shared_ptr<Value> ReduceBuiltin::ngnReduce(std::shared_ptr<Value> x, int axis, std::shared_ptr<Fun> f)
	{ // https://chat.stackexchange.com/transcript/message/47158587#47158587
	  ...
	  std::vector<std::shared_ptr<Value>> r(n0 * n2);
	  for (int i = 0; i < n0; i++)
	  {
		for (int k = 0; k < n2; k++)
		{
		  std::shared_ptr<Value> c = x[i * n1 * n2 + (n1 - 1) * n2 + k];
