...
namespace APL::types::functions::builtins::mops
{
	...
	std::shared_ptr<Value> KeyBuiltin::call(std::shared_ptr<Obj> f, std::shared_ptr<Value> w, std::shared_ptr<DerivedMop> derv)
	{
	...
	std::shared_ptr<Obj> KeyBuiltin::callObj(std::shared_ptr<Obj> aa, std::shared_ptr<Value> w, std::shared_ptr<DerivedMop> derv)
	{
	  if (std::dynamic_pointer_cast<APLMap>(aa) != nullptr)
	  {
		if (w->rank > 1)
		{
		  ...
		  {
			arr[i] = std::static_pointer_cast<Value>((std::static_pointer_cast<APLMap>(aa))->getRaw(w[i]));
		  }
	  ...
	  {
		int i = sc->IO;
		auto vals = std::unordered_map<std::shared_ptr<Value>, std::vector<std::shared_ptr<Value>>>();
		auto order = std::vector<std::shared_ptr<Value>>();
		for (auto v : w)
		{
		  if (vals.find(v) == vals.end())
		  {
			auto l = std::vector<std::shared_ptr<Value>>();
		...
		auto *res = std::vector<std::shared_ptr<Value>>(order.size());
		i = 0;
		for (auto c : order)
		{
	...
	std::shared_ptr<Value> KeyBuiltin::call(std::shared_ptr<Obj> aa, std::shared_ptr<Value> a, std::shared_ptr<Value> w, std::shared_ptr<DerivedMop> derv)
	{
	  if (std::dynamic_pointer_cast<APLMap>(aa) != nullptr)
	  {
	  ...
	  if (std::dynamic_pointer_cast<Fun>(aa) != nullptr)
	  {
		...
		std::unordered_map<std::shared_ptr<Value>, std::vector<std::shared_ptr<Value>>> vals;
		std::vector<std::shared_ptr<Value>> order;
		for (int i = 0; i < a->ia; i++)
		{
		  std::shared_ptr<Value> k = w[i];
		  std::shared_ptr<Value> v = a[i];
		  std::vector<std::shared_ptr<Value>> curr = vals[k];
		  if (curr.empty())
		  {
			std::vector<std::shared_ptr<Value>> newArr;
		...
		std::vector<std::shared_ptr<Value>> res(order.size());
		for (int i = 0; i < order.size(); i++)
		{
		  std::shared_ptr<Value> k = order[i];
		  std::shared_ptr<Value> vs = Arr::create(vals[k]);
		  res[i] = aaf->call(k, vs);
		}
