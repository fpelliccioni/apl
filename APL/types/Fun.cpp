...
namespace APL::types
{
	...
	std::shared_ptr<Value> Fun::identity()
	{
	  return nullptr;
	}
	...
	std::shared_ptr<Value> Fun::call(std::shared_ptr<Value> w)
	{
	  throw IncorrectArgsError(StringHelper::wstring_to_string(L"function " + toString() + L" called monadically", shared_from_this(), w));
	}

	std::shared_ptr<Value> Fun::call(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  throw IncorrectArgsError(StringHelper::wstring_to_string(L"function " + toString() + L" called dyadically", shared_from_this(), a));
	}

	std::shared_ptr<Obj> Fun::callObj(std::shared_ptr<Value> w)
	{ // if overridden, call(w) must be overridden too!
	  return call(w);
	}

	std::shared_ptr<Obj> Fun::callObj(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{ // if overridden, call(a, w) must be overridden too!
	  return call(a, w);
	}

	std::shared_ptr<Value> Fun::callInv(std::shared_ptr<Value> w)
	{
	  throw DomainError(shared_from_this() + L" doesn't support monadic inverting", shared_from_this(), w);
	}

	std::shared_ptr<Value> Fun::callInvW(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  throw DomainError(shared_from_this() + L" doesn't support dyadic inverting of ⍵", shared_from_this(), w);
	}

	std::shared_ptr<Value> Fun::callInvA(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  throw DomainError(shared_from_this() + L" doesn't support dyadic inverting of ⍺", shared_from_this(), w);
	}

	std::shared_ptr<Value> Fun::under(std::shared_ptr<Obj> o, std::shared_ptr<Value> w)
	{
	...
	std::shared_ptr<Value> Fun::underW(std::shared_ptr<Obj> o, std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	...
	std::shared_ptr<Value> Fun::underA(std::shared_ptr<Obj> o, std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  std::shared_ptr<Value> v = std::dynamic_pointer_cast<Fun>(o) != nullptr? (std::static_pointer_cast<Fun>(o))->call(call(a, w)) : std::static_pointer_cast<Value>(o);
	  return callInvA(v, w);
	}
	...
	std::shared_ptr<Value> Fun::NumMV::call(std::shared_ptr<BigValue> w)
	{
	  throw DomainError(L"bigintegers not allowed here", w);
	}

	std::shared_ptr<Arr> Fun::ChrMV::call(std::shared_ptr<ChrArr> a)
	{
	  Value[] res = std::vector<std::shared_ptr<Value>>(a.ia);
	  for (int i = 0; i < a.ia; i++)
		  res[i] = call(Char.of(a.s.charAt(i)));
	  return std::make_shared<HArr>(res, a.shape);
	}

	std::shared_ptr<Value> Fun::allM(std::shared_ptr<AllMV> f, std::shared_ptr<Value> w)
	{
	...
	std::shared_ptr<Value> Fun::numM(std::shared_ptr<NumMV> nf, std::shared_ptr<Value> w)
	{
	  ...
	  {
		  return nf->call(std::static_pointer_cast<BigValue>(w));
	  }
	  throw DomainError(StringHelper::wstring_to_string(L"Expected number, got " + w->humanType(false), shared_from_this(), w));
	}

	std::shared_ptr<Value> Fun::numChrM(std::shared_ptr<NumMV> nf, std::shared_ptr<ChrMV> cf, std::shared_ptr<Value> w)
	{
	  if (std::dynamic_pointer_cast<Arr>(w) != nullptr)
	  {
		...
		{
			return cf->call(std::static_pointer_cast<ChrArr>(w));
		}
		std::shared_ptr<Arr> o = std::static_pointer_cast<Arr>(w);
		std::vector<std::shared_ptr<Value>> arr(o->ia);
		for (int i = 0; i < o->ia; i++)
	...
	std::shared_ptr<Value> Fun::numChrMapM(std::shared_ptr<NumMV> nf, std::shared_ptr<ChrMV> cf, std::shared_ptr<MapMV> mf, std::shared_ptr<Value> w)
	{
	  if (std::dynamic_pointer_cast<Arr>(w) != nullptr)
	  {
		if (w->quickDoubleArr())
	...
	std::shared_ptr<Value> Fun::allD(std::shared_ptr<D_AA> f, std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  ...
	  if (a->scalar())
	  {
		std::shared_ptr<Value> af = a->first();
		...
		{ // ⍺ ⍵¨
		  ...
		  APL::types::Value::const_iterator wi = w->begin();
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	  ...
	  else
	  {
		if (w->scalar())
		{ // ⍺¨ ⍵
		  std::shared_ptr<Value> wf = w->first();

		  std::vector<std::shared_ptr<Value>> arr(a->ia);
		  APL::types::Value::const_iterator ai = a->begin();
		  for (int i = 0; i < a->ia; i++)
		  {
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	...
	std::shared_ptr<Value> Fun::D_NNeN::call(double a, double w)
	{
	  return std::make_shared<Num>(on(a, w));
	}

	std::shared_ptr<Value> Fun::D_NNeN::call(std::vector<double> &a, std::vector<double> &w, std::vector<int> &sh)
	{
	...
	std::shared_ptr<Value> Fun::D_NNeN::call(double a, std::vector<double> &w, std::vector<int> &sh)
	{
	  std::vector<double> res(w.size());
	  on(res, a, w);
	  return std::make_shared<DoubleArr>(res, sh);
	...
	std::shared_ptr<Value> Fun::D_NNeN::call(std::vector<double> &a, double w, std::vector<int> &sh)
	{
	...
	std::shared_ptr<Value> Fun::D_NNeN::call(std::shared_ptr<BigValue> a, std::shared_ptr<BigValue> w)
	{
	  throw DomainError(L"bigintegers not allowed here", w);
	}

	std::shared_ptr<Value> Fun::D_NNeB::call(double a, double w)
	{
	  return on(a, w)? Num::ONE : Num::ZERO;
	}

	std::shared_ptr<Value> Fun::D_NNeB::call(std::vector<double> &a, std::vector<double> &w, std::vector<int> &sh)
	{
	...
	std::shared_ptr<Value> Fun::D_NNeB::call(double a, std::vector<double> &w, std::vector<int> &sh)
	{
	  std::shared_ptr<BitArr::BA> res = std::make_shared<BitArr::BA>(sh);
	  on(res, a, w);
	  return res->finish();
	...
	std::shared_ptr<Value> Fun::D_NNeB::call(std::vector<double> &a, double w, std::vector<int> &sh)
	{
	  std::shared_ptr<BitArr::BA> res = std::make_shared<BitArr::BA>(sh);
	  on(res, a, w);
	  return res->finish();
	...
	std::shared_ptr<Value> Fun::D_NNeB::call(std::shared_ptr<BigValue> a, std::shared_ptr<BigValue> w)
	{
	  throw DomainError(L"bigintegers not allowed here", w);
	}

	std::shared_ptr<Value> Fun::D_NN::call(double a, std::shared_ptr<BigValue> w)
	{ // special requirement for log; only needs to be handled in numD
	  return call(std::make_shared<BigValue>(a), w);
	}

	std::shared_ptr<Value> Fun::numD(std::shared_ptr<D_NN> f, std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  if (a->scalar())
	  {
		if (w->scalar())
		{ // ⊃⍺ ⊃⍵
		  if (std::dynamic_pointer_cast<Primitive>(a) != nullptr & std::dynamic_pointer_cast<Primitive>(w) != nullptr)
		  {
			...
			if ((std::dynamic_pointer_cast<BigValue>(a) != nullptr | an) & (std::dynamic_pointer_cast<BigValue>(w) != nullptr | wn))
			{
			  if (an)
			  {
				  return f->call((std::static_pointer_cast<Num>(a))->num, std::static_pointer_cast<BigValue>(w));
			  }
			  else
			  {
				  return f->call(std::static_pointer_cast<BigValue>(a), wn? std::make_shared<BigValue>((std::static_pointer_cast<Num>(w))->num) : std::static_pointer_cast<BigValue>(w));
			  }
			}
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	...
	std::shared_ptr<Value> Fun::bitD(std::shared_ptr<D_NN> n, std::shared_ptr<D_BB> b, std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  if (a->scalar())
	  {
		if (w->scalar())
		{ // ⊃⍺ ⊃⍵
		  if (std::dynamic_pointer_cast<Primitive>(a) != nullptr & std::dynamic_pointer_cast<Primitive>(w) != nullptr)
		  {
			...
			if ((std::dynamic_pointer_cast<BigValue>(a) != nullptr | an) & (std::dynamic_pointer_cast<BigValue>(w) != nullptr | wn))
			{
			  return n->call(an? std::make_shared<BigValue>((std::static_pointer_cast<Num>(a))->num) : std::static_pointer_cast<BigValue>(a), wn? std::make_shared<BigValue>((std::static_pointer_cast<Num>(w))->num) : std::static_pointer_cast<BigValue>(w));
			}
			throw DomainError(StringHelper::wstring_to_string(L"calling a number-only function with " + w->humanType(true), shared_from_this()));
		  }
		  else
		...
		{ // ⍺¨ ⍵
		  ...
		  {
			if (std::dynamic_pointer_cast<BitArr>(w) != nullptr && Main::isBool(a))
			{
			  return b->call(Main::bool_Keyword(a), std::static_pointer_cast<BitArr>(w));
			}
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	  ...
	  else
	  {
		...
		{ // ⍺ ⍵¨
		  if (std::dynamic_pointer_cast<Primitive>(w) != nullptr)
		  {
			if (std::dynamic_pointer_cast<BitArr>(a) != nullptr && Main::isBool(w))
			{
			  return b->call(std::static_pointer_cast<BitArr>(a), Main::bool_Keyword(w));
			}
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
		...
		{ // ⍺ ¨ ⍵
		  ...
		  {
			return b->call(std::static_pointer_cast<BitArr>(a), std::static_pointer_cast<BitArr>(w));
		  }
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	...
	std::shared_ptr<Value> Fun::numChrD(std::shared_ptr<D_NN> n, std::shared_ptr<D_CC> c, std::shared_ptr<D_AA> def, std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  if (a->scalar())
	  {
		if (w->scalar())
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	...
	std::shared_ptr<Value> Fun::ncbaD(std::shared_ptr<D_NN> n, std::shared_ptr<D_BB> b, std::shared_ptr<D_CC> c, std::shared_ptr<D_AA> def, std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	  if (a->scalar())
	  {
		if (w->scalar())
		{ // ⊃⍺ ⊃⍵
		  if (std::dynamic_pointer_cast<Primitive>(a) != nullptr & std::dynamic_pointer_cast<Primitive>(w) != nullptr)
		  {
			...
			else if ((std::dynamic_pointer_cast<BigValue>(a) != nullptr | an) & (std::dynamic_pointer_cast<BigValue>(w) != nullptr | wn))
			{
			  return n->call(an? std::make_shared<BigValue>((std::static_pointer_cast<Num>(a))->num) : std::static_pointer_cast<BigValue>(a), wn? std::make_shared<BigValue>((std::static_pointer_cast<Num>(w))->num) : std::static_pointer_cast<BigValue>(w));
			}
			else
...
//JAVA TO C++ CONVERTER TODO TASK: Java iterators are only converted within the context of 'while' and 'for' loops:
	...
	bool Fun::equals(std::shared_ptr<Obj> o)
	{
