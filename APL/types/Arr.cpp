...
namespace APL::types
{
	...
	Arr::Arr(std::vector<int> &shape) : Value(shape)
	{
	}

	Arr::Arr(std::vector<int> &shape, int ia) : Value(shape, ia, shape.size())
	{
	}

	Arr::Arr(std::vector<int> &shape, int ia, int rank) : Value(shape, ia, rank)
	{
	}

	std::wstring Arr::string(bool quote)
	{
	  if (rank == 1)
	  { // strings
	...
	std::wstring Arr::toString()
	{
	  if (ia == 0)
	  {
		std::wstring mr = std::dynamic_pointer_cast<Char>(safePrototype()) != nullptr? L"''" : L"⍬";
		if (rank == 1)
		{
			return mr;
		}
		...
		{
		  std::wstring s = Main::formatAPL(shape);
		  return s + L"⍴" + mr;
		}
	  }
	  std::wstring qs = string(Main::quotestrings || Main::noBoxing);
	  if (qs != L"")
	  {
		  return qs;
	  }
...
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
		...
		if (ia == 1)
		{
		  std::shared_ptr<Value> c = this->operator[](0);
		  if (std::dynamic_pointer_cast<Primitive>(c) != nullptr || rank > 2)
		  {
			...
			std::wstring pre = Main::formatAPL(shape);
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
...
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
		...
		{
		  bool charmat = true;
		  if (!(std::dynamic_pointer_cast<ChrArr>(shared_from_this()) != nullptr))
		  {
			for (auto v : shared_from_this())
			{
			  if (!(std::dynamic_pointer_cast<Char>(v) != nullptr))
...
//ORIGINAL LINE: String[][][] stringified = new String[w][h][];
		  std::vector<std::vector<std::vector<std::wstring>>> stringified = RectangularVectors::RectangularStdWstringVector(w, h, -1);
//JAVA TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in Java:
...
//ORIGINAL LINE: char[][] chars = new char[rh][rw];
		  ...
		  for (x = 0; x < w; x++)
		  {
			...
			for (y = 0; y < h; y++)
			{
			  std::vector<std::wstring> cobj = stringified[x][y];
			  for (int cy = 0; cy < cobj.size(); cy++)
			  {
				std::wstring s = cobj[cy];
	...
	std::wstring Arr::oneliner(std::vector<int> &where)
	{
	  auto qs = string(true);
	  if (qs != L"")
	  {
	...
	std::shared_ptr<Value> Arr::createL(std::vector<std::shared_ptr<Value>> &v, std::vector<int> &sh)
	...
	std::shared_ptr<Arr> Arr::create(std::vector<std::shared_ptr<Value>> &v)
	{
	  return create(v, std::vector<int>{v.size()});
	}

	std::shared_ptr<Arr> Arr::create(std::vector<std::shared_ptr<Value>> &v, std::vector<int> &sh)
	{ // note, doesn't attempt individual item squeezing
	  ...
	  if (std::dynamic_pointer_cast<Char>(v[0]) != nullptr)
	  {
		...
		if (s != nullptr)
		{
			return std::make_shared<ChrArr>(s->toString(), sh);
		}
	  }
	...
	bool Arr::equals(std::shared_ptr<Obj> o)
	{
	  ...
	  std::vector<std::shared_ptr<Value>> mvs = values();
	  std::vector<std::shared_ptr<Value>> ovs = a->values();
	...
	void Arr::eqShapes(std::shared_ptr<Value> a, std::shared_ptr<Value> w)
	{
	...
	void Arr::eqShapes(std::vector<int> &a, std::vector<int> &w, std::shared_ptr<Callable> blame)
	{
	  if (a.size() != w.size())
	  {
