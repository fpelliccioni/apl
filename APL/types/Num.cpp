...
const std::vector<wchar_t> Num::buf = std::vector<wchar_t>(400);
	...
	std::wstring Num::format(double d)
	{
	  double a = std::abs(d);
	  if (d == 0)
	  {
	  ...
	  std::wstring f = String::format(Locale::ENGLISH, L"%." + std::to_wstring(pp) + L"e", a);
	...
	std::wstring Num::formatInt(int i)
	{
...
const std::shared_ptr<Num> Num::NEG_ZERO = std::make_shared<Num>(-0.0);
	...
	bool Num::equals(std::shared_ptr<Obj> n)
	{
	...
	std::wstring Num::toString()
	{
	  return format(num);
	}

	std::wstring Num::oneliner(std::vector<int> &ignored)
	{
	  return format(num);
	}

	std::shared_ptr<Value> Num::ofShape(std::vector<int> &sh)
	{
	...
	std::shared_ptr<Value> Num::safePrototype()
	{
	  return ZERO;
	}

	std::vector<std::shared_ptr<Value>> Num::valuesCopy()
	{
	  return std::vector<std::shared_ptr<Value>>{shared_from_this()};
	}
