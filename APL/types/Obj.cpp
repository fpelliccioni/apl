...
namespace APL::types
{
    ...
    bool Obj::equals(std::shared_ptr<APL::types::Obj> o)
    {
      return false;
    }

    std::string Obj::humanType(bool article)
    {
      ...
      if (std::dynamic_pointer_cast<APL::types::BigValue>(shared_from_this()) != nullptr)
      {
    ...
    {
      return obj.type() == typeid(std::shared_ptr<APL::types::Obj>) && equals(std::any_cast<std::shared_ptr<APL::types::Obj>>(obj));
    }
    ...
    int Obj::hashCode()
    {
      throw APL::errors::NYIError(StringHelper::wstring_to_string(L"hash not supported for " + shared_from_this(), shared_from_this()));
    }
