...
const std::shared_ptr<APL::types::arrs::ChrArr> Main::alphabet = toAPL(L"ABCDEFGHIJKLMNOPQRSTUVWXYZ");
const std::shared_ptr<APL::types::arrs::ChrArr> Main::lowercaseAlphabet = toAPL(L"abcdefghijklmnopqrstuvwxyz");
const std::shared_ptr<APL::types::arrs::ChrArr> Main::digits = toAPL(L"0123456789");
int Main::printlvl = 0;
std::shared_ptr<Scanner> Main::console;
std::shared_ptr<APL::types::Tokenable> Main::faulty;
std::shared_ptr<APL::errors::APLError> Main::lastError = nullptr;

    void Main::main(std::vector<std::string> &args)
    {
...
//ORIGINAL LINE: try (FileOutputStream stream = new FileOutputStream(conv))
              ...
              std::string qk = p.substr(0, si);
              std::string qv = p.substr(si + 1);
...
const std::shared_ptr<StdSys> Main::StdSys::inst = std::make_shared<StdSys>();
    ...
    void Main::StdSys::println(std::string const& s) {
      std::wcout << s << std::endl;
    }

    void Main::StdSys::print(std::string const& s) {
      std::wcout << s;
    }

    void Main::StdSys::colorprint(std::string const& s, int col) {
      if (colorful) {
    ...
    std::string Main::StdSys::input() {
      return console->nextLine();
    }

    std::string Main::formatAPL(std::vector<int> &ia) {
      if (ia.empty()) {
          return L"⍬";
      }
    ...
    std::string Main::readFile(std::string const& path) {
      return readFile(Paths->get(path));
    }

    std::string Main::readFile(std::shared_ptr<Path> path) {
      try {
        std::vector<signed char> encoded = Files::readAllBytes(path);
        return std::string(encoded, StandardCharsets::UTF_8);
      } catch (const IOException &e) {
        std::string msg = L"File " + path + L" not found";
    ...
    std::shared_ptr<APL::types::Obj> Main::rexec(std::shared_ptr<APL::tokenizer::types::LineTok> s, std::shared_ptr<Scope> sc) {
      return (std::make_shared<Exec>(s, sc))->exec();
    }

    std::shared_ptr<APL::types::Obj> Main::exec(std::string const& s, std::shared_ptr<Scope> sc) {
      std::shared_ptr<APL::tokenizer::types::BasicLines> t = Tokenizer::tokenize(s);
    ...
    std::shared_ptr<APL::types::Obj> Main::exec(std::shared_ptr<APL::tokenizer::types::LineTok> s, std::shared_ptr<Scope> sc) {
      std::shared_ptr<APL::types::Obj> val = (std::make_shared<Exec>(s, sc))->exec();
    ...
    std::shared_ptr<APL::types::Value> Main::vexec(std::string const& s, std::shared_ptr<Scope> sc) {
      std::shared_ptr<APL::types::Obj> val = Main::exec(s, sc);
      if (std::dynamic_pointer_cast<APL::types::Value>(val) != nullptr) {
          return std::static_pointer_cast<APL::types::Value>(val);
      }
      throw SyntaxError(StringHelper::wstring_to_string(L"expected array, got " + val->humanType(true)));
    }

    std::shared_ptr<APL::types::Value> Main::vexec(std::shared_ptr<APL::tokenizer::types::LineTok> s, std::shared_ptr<Scope> sc) {
      std::shared_ptr<APL::types::Obj> val = Main::exec(s, sc);
      if (std::dynamic_pointer_cast<APL::types::Value>(val) != nullptr) {
          return std::static_pointer_cast<APL::types::Value>(val);
      }
    ...
    bool Main::isBool(std::shared_ptr<APL::types::Value> a) {
    ...
    std::shared_ptr<APL::types::Obj> Main::execLines(std::shared_ptr<TokArr<std::shared_ptr<APL::tokenizer::types::LineTok>>> lines, std::shared_ptr<Scope> sc) {
      std::shared_ptr<APL::types::Obj> res = nullptr;
      std::unordered_map<EType, std::shared_ptr<APL::tokenizer::types::LineTok>> eGuards;
      try {
        for (auto ln : lines->tokens) {
          ...
          if (guardPos != -1) {
            auto guard = APL::tokenizer::types::LineTok::inherit(tokens.subList(0, guardPos));
            if (bool_Keyword(vexec(guard, sc))) {
              auto expr = APL::tokenizer::types::LineTok::inherit(tokens.subList(guardPos + (endAfter? 2 : 1), tokens.size()));
              res = exec(expr, sc);
              if (endAfter) {
                  return res;
              }
            ...
            auto guard = APL::tokenizer::types::LineTok::inherit(tokens.subList(0, eguardPos));
            std::shared_ptr<APL::types::Value> r = vexec(guard, sc);
            EType t;
            ...
            auto expr = APL::tokenizer::types::LineTok::inherit(tokens.subList(eguardPos + (endAfter? 2 : 1), tokens.size()));
            eGuards.emplace(t, expr);
          } else {
            res = exec(endAfter? APL::tokenizer::types::LineTok::inherit(tokens) : ln, sc);
            if (endAfter) {
                return res;
            }
          }
    ...
    bool Main::bool_Keyword(std::shared_ptr<APL::types::Obj> v) {
    ...
    std::shared_ptr<APL::types::arrs::ChrArr> Main::toAPL(std::string const& s) {
      return std::make_shared<APL::types::arrs::ChrArr>(s);
    }

    std::string Main::repeat(std::string const& s, int l) {
      std::shared_ptr<StringBuilder> r = std::make_shared<StringBuilder>();
      for (int i = 0; i < l; i++) {
          r->append(s);
      }
