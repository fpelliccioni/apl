...
//	  Type(char chr)
	...
	{
	private:
		static std::vector<std::pair<Type, std::wstring>> pairs()
		{
			return
			{
				{Type::array, L"array"},
	...
	public:
		...
		static std::wstring enumName(Type value)
		{
			for (auto pair : pairs())
			{
				if (pair.first == value)
		...
		{
			std::vector<std::pair<Type, std::wstring>> temp = pairs();
		...
		static Type enumFromString(std::wstring value)
		{
			for (auto pair : pairs())
			{
