...
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
    ...
    void Sys::ucmd(std::string const& cr)
    {
      std::vector<std::string> parts = cr.split(L" ");
      std::string t = StringHelper::toUpper(parts[0]);
      std::string rest = parts.size() == 1? L"" : cr.substr(t.length() + 1);
...
//ORIGINAL LINE: case "EX":
      ...
      {
          std::string full = cr.substr((int)cr.find(L" ") + 1);
...
//ORIGINAL LINE: case "CLASS" :
      ...
      {
            std::shared_ptr<APL::types::Value> r = Main::vexec(rest, csc);
...
//ORIGINAL LINE: case "CS":
      else if (t == L"CS")
      {
          ...
          {
            std::shared_ptr<APL::types::Obj> o = Main::exec(rest, csc);
            std::shared_ptr<Scope> nsc = nullptr;
            if (std::dynamic_pointer_cast<APL::types::Callable>(o) != nullptr)
            {
                nsc = (std::static_pointer_cast<APL::types::Callable>(o))->sc;
            }
            if (nsc == nullptr)
            {
    ...
    std::shared_ptr<Path> Sys::path(std::string const& path, std::string const& s)
    {
      if (path == L"")
      {
    ...
    std::vector<std::string> Sys::split(std::shared_ptr<Path> p)
    {
      p = p->toAbsolutePath().normalize();
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
      return std::vector<std::string>{p->getParent()->toString() + L"/", p->getFileName()->toString()};
    }

    std::shared_ptr<APL::types::Obj> Sys::execFile(std::shared_ptr<Path> path, std::shared_ptr<Scope> sc)
    {
      std::string code = Main::readFile(path);
      return Main::exec(code, sc);
    }

    void Sys::line(std::string const& s)
    {
      ...
      {
        std::shared_ptr<APL::types::Obj> res = Main::exec(s, csc);
        if (res != nullptr)
        {
            println(res);
        }
    ...
    void Sys::lineCatch(std::string const& s)
    {
      try
      {
        line(s);
    ...
    {
      Main::lastError = std::dynamic_pointer_cast<APL::errors::APLError>(t) != nullptr? std::static_pointer_cast<APL::errors::APLError>(t) : std::make_shared<ImplementationError>(t);
      Main::lastError->print(shared_from_this());
    }

    void Sys::colorprint(std::string const& s, int col)
    {
      println(s);
    }

    void Sys::println(std::shared_ptr<APL::types::Obj> v)
    {
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
      println(v->toString());
    }
