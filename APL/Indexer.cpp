...
namespace APL
{
	...
	int Indexer::fromShapeChk(std::vector<int> &sh, std::vector<int> &pos, std::shared_ptr<Callable> blame)
	{ // IO≡0
	  if (sh.size() != pos.size())
	  {
	...
	int Indexer::fromShapeChk(std::vector<int> &sh, std::shared_ptr<Value> pos, int IO, std::shared_ptr<Callable> blame)
	{
	  if (pos->rank > 1)
	  {
	...
	std::shared_ptr<PosSh> Indexer::poss(std::shared_ptr<Value> v, std::vector<int> &ish, int IO, std::shared_ptr<Callable> blame)
	{
	  ...
	  { // ⎕VI←1
		bool deep = false;
		std::vector<int> rsh;
		if (!(std::dynamic_pointer_cast<DoubleArr>(v) != nullptr || std::dynamic_pointer_cast<ChrArr>(v) != nullptr || std::dynamic_pointer_cast<BitArr>(v) != nullptr))
		{
		  for (auto c : v)
		  {
	...
	std::vector<int> Indexer::intsNoIO(std::shared_ptr<Value> v, int IO)
	{
	  if (IO == 0)
	  {
		  return v->asIntArr();
