...
namespace APL
{
	...
	Exec::Exec(std::shared_ptr<LineTok> ln, std::shared_ptr<Scope> sc) : sc(sc), tokens(ln->tokens), allToken(ln)
	{
	}

	void Exec::printlvl(std::vector<std::any> &args)
	{
	  if (!Main::debug)
	  {
		  return;
	...
	std::shared_ptr<Obj> Exec::exec()
	{
	  ...
	  while (left.size() > 0)
	  {
		...
		if (std::dynamic_pointer_cast<NameTok>(t) != nullptr && left.size() >= 2 && std::dynamic_pointer_cast<OpTok>(left.top()) != nullptr && (std::static_pointer_cast<OpTok>(left.top()))->op == L"." && std::dynamic_pointer_cast<NameTok>(left.get(left.size() - 2)) != nullptr)
		{
		  ...
		  std::vector<std::wstring> names((left.size() - ptr >> 1) + 1);
...
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
	...
	void Exec::update(bool end)
	{
	  ...
	  while (llSize != 1)
	  {
		...
		{
		  if (is(std::vector<std::wstring>{L"D!|V←[#NFMD]", L"#←[#NFMDV]", L"D!|D←D", L"D!|M←M", L"D!|F←F", L"D!|N←N"}, end, false))
		  ...
		  if (is(L"D!|NF←N", end, false, 5))
		  {
			...
			auto s = std::static_pointer_cast<SetBuiltin>(s0);
			auto f = lastFun();
			std::shared_ptr<Obj> a = popE(); // variable
			Main::faulty = f;
			std::shared_ptr<Obj> res = s->callObj(f, a, w);
			if (res != nullptr)
			{
				addE(res);
			}
	  ...
	  if (end && llSize == 2)
	  {
		...
		auto h = lastFun();
		auto g = lastObj();
		if (std::dynamic_pointer_cast<Fun>(g) != nullptr || std::dynamic_pointer_cast<Value>(g) != nullptr)
		{
			addE(std::make_shared<Atop>(g, h));
		}
		else
	...
	std::shared_ptr<Value> Exec::lastVal()
	{
	  auto r = popE();
	  ...
	  if (std::dynamic_pointer_cast<Value>(r) != nullptr)
	  {
		  return std::static_pointer_cast<Value>(r);
	  }
	...
	std::shared_ptr<Value> Exec::firstVal()
	{
	  auto r = popB();
	  if (std::dynamic_pointer_cast<Settable>(r) != nullptr)
	  {
	...
	std::shared_ptr<Obj> Exec::lastObj()
	{
	  auto r = popE();
	  if (std::dynamic_pointer_cast<Settable>(r) != nullptr)
	  {
	...
	std::shared_ptr<Obj> Exec::firstObj()
	{
	  auto r = popB();
	  if (std::dynamic_pointer_cast<VarArr>(r) != nullptr)
	  {
	...
	void Exec::addFirst(std::shared_ptr<Obj> o)
	{
	  addB(o);
	}

	std::shared_ptr<Obj> Exec::popS()
	{
	  llSize--;
	  std::shared_ptr<Node> c = FN->r;
	  std::shared_ptr<Node> r = c->r;
	  std::shared_ptr<Obj> res = c->val;
	  FN->r = c->r;
	  r->l = FN;
	  return res;
	}

	std::shared_ptr<Obj> Exec::popE()
	{
	  llSize--;
	  std::shared_ptr<Node> c = LN->l;
	  std::shared_ptr<Node> l = c->l;
	  std::shared_ptr<Obj> r = c->val;
	  LN->l = c->l;
	  l->r = LN;
	  return r;
	}

	std::shared_ptr<Obj> Exec::popB()
	{
	  llSize--;
	  std::shared_ptr<Obj> r = barNode->remove();
	  barNode = barNode->r;
	  return r;
	}

	void Exec::addS(std::shared_ptr<Obj> o)
	{
	  llSize++;
	...
	void Exec::addE(std::shared_ptr<Obj> o)
	{
	  llSize++;
	...
	void Exec::addB(std::shared_ptr<Obj> o)
	{
	  llSize++;
	...
	std::shared_ptr<Obj> Exec::pollS()
	{
	  return FN->r->val;
	}

	std::shared_ptr<Obj> Exec::pollL()
	{
	  return LN->l->val;
	}
	...
	Exec::Node::Node(std::shared_ptr<Obj> val, std::shared_ptr<Node> l, std::shared_ptr<Node> r)
	{
	  this->l = l;
	  this->r = r;
	  this->val = val;
	...
	std::shared_ptr<Obj> Exec::Node::remove()
	{
	  l->r = r;
	  r->l = l;
	  return val;
	...
	std::wstring Exec::Node::toString()
	{
...
//JAVA TO C++ CONVERTER TODO TASK: There is no C++ equivalent to 'toString':
	...
	bool Exec::is(std::vector<std::wstring> &pts, bool everythingDone, bool fromStart)
	{
	  for (auto pt : pts)
	  {
	...
	bool Exec::is(const std::wstring &pt, bool everythingDone, bool fromStart)
	{
	  return is(pt, everythingDone, fromStart, 4);
	}

	bool Exec::is(const std::wstring &pt, bool everythingDone, bool fromStart, int am)
	{
	...
	std::shared_ptr<Obj> Exec::valueOf(std::shared_ptr<Token> t)
	{
	  std::shared_ptr<Obj> o = valueOfRaw(t);
	  o->token = t;
	  return o;
	}

	std::shared_ptr<Obj> Exec::valueOfRaw(std::shared_ptr<Token> t)
	{
	  if (std::dynamic_pointer_cast<OpTok>(t) != nullptr)
	  {
		...
		switch (t1->op[0])
		{
		  ...
		  case L'⍹':
			  ...
			  return o;
		  default:
			  throw NYIError(StringHelper::wstring_to_string(L"no built-in " + (std::static_pointer_cast<OpTok>(t))->op + L" defined in exec", t));
		}
	  }
	  ...
	  if (std::dynamic_pointer_cast<LineTok>(t) != nullptr)
	  {
		  return Main::rexec(std::static_pointer_cast<LineTok>(t), sc);
	  }
	  if (std::dynamic_pointer_cast<ParenTok>(t) != nullptr)
	  {
		std::vector<std::shared_ptr<LineTok>> ts = (std::static_pointer_cast<ParenTok>(t))->tokens;
		int size = ts.size();
		if (size == 0)
		{
			return std::make_shared<StrMap>();
		}
		std::shared_ptr<LineTok> fst = ts[0];
		...
		{ // map constants
		  ...
		  for (auto ct : ts)
		  {
			...
			std::shared_ptr<Obj> val = Main::exec(LineTok::inherit(tokens), nsc);
			res->setStr(key, val);
		  }
		  return res;
		}
		...
		{ // array
		  std::shared_ptr<Obj> fo = Main::exec(fst, sc);
		  if (std::dynamic_pointer_cast<Value>(fo) != nullptr)
		  { // value array
			std::vector<std::shared_ptr<Value>> vs(size);
			for (int i = 0; i < ts.size(); i++)
			{
			  std::shared_ptr<Obj> o = i == 0? fo : Main::exec(ts[i], sc);
			  ...
			  vs[i] = std::static_pointer_cast<Value>(o);
			}
			return Arr::create(vs);
		  }
		  else if (std::dynamic_pointer_cast<Fun>(fo) != nullptr)
		  { // function array
			std::vector<std::shared_ptr<Obj>> os(size);
			for (int i = 0; i < ts.size(); i++)
			{
			  std::shared_ptr<Obj> o = i == 0? fo : Main::exec(ts[i], sc);
	  ...
	  if (std::dynamic_pointer_cast<BracketTok>(t) != nullptr)
	  {
		  return Brackets::of(std::static_pointer_cast<BracketTok>(t), sc);
	  }
	  ...
	  throw NYIError(StringHelper::wstring_to_string(L"Unknown type: " + t->toRepr(), t));
	}
}
