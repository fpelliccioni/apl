...
// protected:
    APLError(std::string msg, T blame)
...
//    std::vector<Mg> l = new std::vector<>();
...
//     else for (Mg g : l) {
//      std::vector<Mg> c = new std::vector<>();
//       c.add(g);
//       println(s, c);
//     }
//   }

//   public void println(Sys s, std::vectorMg> gs) {
//     if (gs.size() == 0) return;
...
//     static void add(std::vector<Mg> l, Tokenable to, char c) {
//       Token t = to.getToken();
//       if (t == null) return;
