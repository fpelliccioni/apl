...
public abstract class Fun extends Callable {
  ...
  public abstract static class D_NNeB implements D_NN { // dyadic number-number equals bool
    public abstract bool on(double a, double w);
  ...
  public interface D_BB {
    ...
    Value call(bool a, BitArr  w);
    Value call(BitArr  a, bool w);
  }
    ...
    if (a.scalar()) {
      if (w.scalar()) { // ⊃⍺ ⊃⍵
        if (a instanceof Primitive & w instanceof Primitive) {
          bool an = a instanceof Num;
          bool wn = w instanceof Num;
