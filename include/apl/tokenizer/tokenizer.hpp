...
public:
    ...
    public:
        ...
        void add(token r) {
            if (annoyingBacktickPos) {
                ts.push_back(types::backtick(line, *annoyingBacktickPos, epos(r), r));
        ...
        types::line tok() {
            ...
            return types::line(line, spos_, epos_, ts);
        }
    };
    ...
    static
    types::basic_lines tokenize(std::string raw) {
        return tokenize(raw, false);
    }

    static
    token chr_to_token(char c, std::string raw, Block closed, int i, std::vector<types::line> lineTokens) {
        switch (c) {
          case ')':
            return types::paren(raw, closed.pos, i + 1, lineTokens, closed.hasDmd);
          case '}':
            return types::dfn(raw, closed.pos, i + 1, lineTokens);
          case ']':
            return types::bracket(raw, closed.pos, i + 1, lineTokens, closed.hasDmd);
            break;
          default:
    ...
    static
    types::basic_lines tokenize(std::string raw, bool pointless) { // pointless means unevaled things get tokens; mainly for syntax highlighting
      int li = 0;
      int len = raw.length();
      ...
      for (int i = 0; i < len; li = i) {
          Block expr = levels.at(levels.size() - 1);
          std::vector<Line> lines = expr.a;
          Line tokens = lines.at(lines.size() - 1);
          try {
            char c = raw.at(i);
            ...
            if (c == '(' || c == '{' || c == '[') {
              ...
              lines = levels.at(levels.size() - 1).a;
              ...
              std::vector<types::line> lineTokens;
              ...
              token r = chr_to_token(c, raw, closed, i, lineTokens);
              lines = levels.at(levels.size() - 1).a;
              tokens = lines.at(lines.size() - 1);
              tokens.add(r);
              i++;
              ...
              tokens.add(types::name(raw, li, i, name));
            } else if (c=='¯' && next=='∞') {
              i+= 2;
              tokens.add(types::num(raw, li, i, Double.NEGATIVE_INFINITY));
            } else if (c == '∞') {
              i++;
              tokens.add(types::num(raw, li, i, Double.POSITIVE_INFINITY));
      ...
      if (levels.size() != 1) {
        ...
        while (levels.size() > 1) {
          ...
          auto lineTokens = std::vector<types::line>();
          for (Line ta : closed.a) lineTokens.push_back(ta.tok());
          token r;
          switch (closed.b) {
            case ')':
          ...
          auto lines = levels.at(levels.size() - 1).a;
          Line tokens = lines.at(lines.size() - 1);
          tokens.add(r);
        }
      }
      auto lines = levels.at(0).a;
      if (lines.size() > 0 && lines.at(lines.size()-1).size() == 0) lines.remove(lines.size()-1); // no trailing empties!!
      auto expressions = std::vector<types::line>();
      for (Line line : lines) {
